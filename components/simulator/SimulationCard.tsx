...
const STATUS_CONFIG = {
  ...
  void:    { label: 'Anulada', icon: Ban, color: 'text-texto-secundario', bg: 'bg-slate-500/10 border-slate-500/20' },
  cancelled: { label: 'Cancelada', icon: Ban, color: 'text-texto-secundario', bg: 'bg-slate-500/10 border-slate-500/20' },
}
...
export function SimulationCard({ simulation, onResolved }: Props) {
  ...
  return (
    <div className="rounded-xl border border-neon/10 bg-superficie/70 overflow-hidden">
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center gap-3 p-4 text-left hover:bg-superficie-hover/40 transition-colors"
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-texto-terciario">
            ...
            <span className="text-neon">{Number(simulation.potential_virtual_return).toFixed(2)}€</span>
            {' · '}{formatDistanceToNow(new Date(simulation.created_at), { addSuffix: true, locale: es })}
          </p>
        </div>
        {open ? <ChevronUp className="h-4 w-4 text-texto-terciario shrink-0" /> : <ChevronDown className="h-4 w-4 text-texto-terciario shrink-0" />}
      </button>

      {open && (
        <div className="px-4 pb-4 border-t border-neon/10 pt-3 space-y-3">
          <div className="space-y-1.5">
            {simulation.simulation_picks.map(pick => (
              <div key={pick.id} className="flex items-center justify-between text-xs bg-superficie-hover/60 rounded-lg px-3 py-2">
                <div className="min-w-0">
                  <p className="text-white truncate">{pick.event_name}</p>
                  <p className="text-texto-terciario">{pick.league_name} · {pick.selection}</p>
                </div>
          ...
          {simulation.status === 'pending' && (
            <div>
              <p className="text-xs text-texto-terciario mb-2">¿Cómo quedó?</p>
              <div className="grid grid-cols-3 gap-2">
                ...
                <button
                  ...
                  disabled={resolving}
                  className="flex items-center justify-center gap-1 text-xs py-2 rounded-lg bg-superficie-hover/50 border border-slate-600/30 text-texto-secundario hover:bg-superficie-hover transition-colors disabled:opacity-40"
                >
