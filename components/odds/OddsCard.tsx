...
export function OddsCard({ event, selectedPicks, onAddPick, onRemovePick }: OddsCardProps) {
  ...
  return (
    <div className={cn(
      'rounded-xl border bg-superficie/50 p-4 transition-colors',
      isSelected ? 'border-teal-500/50 bg-teal-950/20' : 'border-superficie-hover hover:border-neon'
    )}>
      <div className="flex items-start justify-between gap-2 mb-3">
        <div className="flex-1 min-w-0">
          ...
          <p className="text-texto font-medium text-sm leading-snug">{event.event_name}</p>
          <div className="flex items-center gap-1.5 mt-1 text-xs text-texto-secundario">
            <Clock className="h-3 w-3" />
            <span>{formatDate(event.commence_time)}</span>
            {!started && <span className="text-teal-500">({timeUntil(event.commence_time)})</span>}
          </div>
        </div>
        <span className="text-xs text-texto-terciario shrink-0">{event.bookmakers_count} casas</span>
      </div>
              ...
              <button
                ...
                className={cn(
                  ...
                  isPickSelected
                    ...
                    : 'border-superficie-hover hover:border-teal-500/50 hover:bg-teal-500/5 text-texto-secundario'
                )}
              >
                <span className="text-xs text-texto-secundario mb-0.5">{OUTCOME_LABELS[outcome]}</span>
                <span className="font-bold text-white text-base">{formatOdds(odds)}</span>
                <span className="text-xs text-texto-secundario truncate w-full">{prob}%</span>
                <span className="text-xs text-texto-terciario truncate w-full mt-0.5" title={teamName}>
      ...
      {!isSelected && !started && (
        <button
          ...
          className="mt-2 w-full flex items-center justify-center gap-1 text-xs text-texto-secundario hover:text-teal-400 transition-colors py-1"
        >
