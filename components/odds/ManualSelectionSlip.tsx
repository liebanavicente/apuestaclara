...
export function ManualSelectionSlip({ picks, onRemove, onClear, isLoggedIn }: ManualSelectionSlipProps) {
  ...
  return (
    <div className="fixed bottom-4 right-4 z-40 w-80 rounded-xl border border-teal-500/40 bg-slate-900 shadow-2xl shadow-black/50">
      ...
      <div className="p-3 space-y-2 max-h-52 overflow-y-auto">
        {picks.map(pick => (
          <div key={pick.event_id} className="flex items-start justify-between gap-2 bg-slate-800/60 rounded-lg p-2.5">
            <div className="flex-1 min-w-0">
              <p className="text-xs text-teal-400 truncate">{pick.league}</p>
              <p className="text-xs text-slate-300 truncate">{pick.event_name}</p>
              <p className="text-xs text-slate-400">{pick.market} · <strong className="text-white">{pick.selection}</strong></p>
            </div>
            <div className="flex items-center gap-1.5 shrink-0">
              <span className="text-white font-bold text-sm">{formatOdds(pick.odds)}</span>
              <button onClick={() => onRemove(pick.event_id)} className="text-slate-600 hover:text-red-400 transition-colors">
      ...
      <div className="px-4 py-3 border-t border-slate-800">
        ...
        <div className="flex justify-between text-xs mb-3">
          <span className="text-slate-400">Prob. implícita</span>
          <span className="text-slate-300">{impliedProb}%</span>
        </div>
