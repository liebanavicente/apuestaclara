...
export default async function RankingPage() {
  ...
  return (
    <main className="max-w-2xl mx-auto px-4 py-10">
      ...
      <div className="flex gap-4 text-xs text-texto-terciario mb-8 pl-1">
          ...
          {[players[1], players[0], players[2]].map((p: any, podiumIdx) => {
            ...
            return (
              <div key={p.user_id} className={`rounded-xl border text-center p-3 ${
                rank === 1 ? 'border-ambar/40 bg-ambar/5 col-start-2' :
                rank === 2 ? 'border-superficie-hover/40 bg-superficie/50 col-start-1 row-start-1 self-end' :
                ...
                <p className={`font-black mt-0.5 ${rank === 1 ? 'text-ambar text-lg' : 'text-white text-base'}`}>{(+p.total_points || 0).toFixed(2)}</p>
                <p className="text-xs text-texto-secundario">{p.total_won ?? 0}/{p.total_resolved ?? 0} ✓</p>
              </div>
            )
          })}
        </div>
      ...
      <div className="space-y-2">
        {players.length === 0 && (
          <div className="text-center py-16 text-texto-secundario">
            ...
            <Link href="/dashboard" className="text-neon text-sm mt-2 block">Hacer picks →</Link>
          </div>
        )}
        {players.map((p: any, i: number) => {
          const isMe = user && p.user_id === user.id
          return (
            <div key={p.user_id} className={`flex items-center gap-3 rounded-xl border px-4 py-3 ${isMe ? 'border-neon/40 bg-neon/5' : 'border-superficie-hover bg-superficie/40'}`}>
