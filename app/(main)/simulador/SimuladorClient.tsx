    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="space-y-4">
        {wallet ? (
          ...
          <div className="rounded-xl border border-neon/10 bg-superficie/80 p-5 animate-pulse h-40" />
        )}
        ...
        <div className="rounded-xl border border-neon/10 bg-superficie/80 p-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div key={label} className="bg-superficie-hover/60 rounded-lg p-2.5">
                <div className="flex items-center gap-1 mb-1">
                  <Icon className={`h-3 w-3 ${color}`} />
                  <span className="text-xs text-texto-terciario">{label}</span>
                </div>
          ...
          {simulations.length > 0 && (
            <div className="mt-3 pt-3 border-t border-neon/10">
              <p className="text-xs text-texto-terciario">Cuota media</p>
        ...
        {isLoggedIn ? (
          ...
          <Link
            href="/register"
            className="w-full flex items-center justify-center gap-2 bg-neon hover:brightness-110 text-carbon font-semibold py-3 rounded-xl transition-colors"
          >
      ...
      <div className="lg:col-span-2">
        {/* Tabs */}
        <div className="flex gap-1 mb-4 bg-superficie/80 rounded-xl p-1 border border-neon/10">
          {([
            ...
            <button
              ...
              className={`flex-1 text-sm py-2 rounded-lg transition-colors ${
                activeTab === tab.id ? 'bg-neon text-carbon' : 'text-texto-secundario hover:text-white'
              }`}
            >
              {tab.label}
            </button>
        ...
        {loading ? (
          <div className="space-y-3">
            {[1, 2, 3].map(i => (
              <div key={i} className="rounded-xl border border-neon/10 h-16 animate-pulse bg-superficie/70" />
            ))}
          </div>
        ) : (
          <div className="space-y-2">
            ...
            {activeTab === 'resolved' && (
              resolvedList.length === 0 ? (
                <div className="rounded-xl border border-neon/10 bg-superficie/60 p-12 text-center">
                  <p className="text-texto-terciario">Aún no has resuelto ninguna simulación.</p>
                </div>
              ) : (
