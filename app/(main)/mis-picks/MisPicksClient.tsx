...
import { Plus, Check, X, Clock, Trophy, Search } from 'lucide-react'
...
export function MisPicksClient({ picks, bankroll }: Props) {
  ...
  const canSubmit = !loading && !!form.description && !!form.selection && !!form.odds && !!form.stake
        ...
        <form onSubmit={handleSubmit} className="rounded-xl border border-yellow-500/30 bg-slate-900 p-5 mb-6 space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="flex rounded-lg border border-slate-700 overflow-hidden text-xs">
              <button type="button" onClick={() => { setInputMode('select'); if (events.length === 0) loadEvents() }}
                className={`px-3 py-1.5 transition-colors ${inputMode === 'select' ? 'bg-yellow-500 text-slate-950 font-bold' : 'text-slate-400 hover:text-white'}`}>
                📅 Elegir partido
              </button>
              <button type="button" onClick={() => setInputMode('manual')}
                className={`px-3 py-1.5 transition-colors ${inputMode === 'manual' ? 'bg-yellow-500 text-slate-950 font-bold' : 'text-slate-400 hover:text-white'}`}>
                ✏️ Manual
              </button>
            </div>
          </div>
          ...
          {inputMode === 'select' && (
            <div className="space-y-3">
              ...
              {eventsLoading ? (
                ...
                <div className="max-h-52 overflow-y-auto space-y-1.5 pr-1">
                  {filteredEvents.map(ev => (
                    <button key={ev.id} type="button" onClick={() => selectEvent(ev)}
                      className={`w-full text-left rounded-lg border px-3 py-2.5 transition-colors ${
                        selectedEvent?.id === ev.id ? 'border-yellow-500/50 bg-yellow-500/10' : 'border-slate-700 bg-slate-800 hover:border-slate-600'
                      }`}>
              ...
              {selectedEvent && (
                <div className="space-y-3">
                  <div className="flex rounded-lg border border-slate-700 overflow-hidden text-xs">
                    <button type="button" onClick={() => { setMarket('result'); setForm(f => ({ ...f, selection: '', odds: '' })) }}
                      className={`flex-1 px-3 py-1.5 transition-colors ${market === 'result' ? 'bg-slate-700 text-white font-bold' : 'text-slate-400 hover:text-white'}`}>
                      ⚽ Resultado (1X2)
                    </button>
                    <button type="button" onClick={() => { setMarket('custom'); setForm(f => ({ ...f, selection: '', odds: '' })) }}
                      className={`flex-1 px-3 py-1.5 transition-colors ${market === 'custom' ? 'bg-slate-700 text-white font-bold' : 'text-slate-400 hover:text-white'}`}>
                      🎲 Goles / Goleador / Otro
                    </button>
                  </div>
                    ...
                    <div className="rounded-lg border border-slate-700 bg-slate-800 p-3">
                      <div className="grid grid-cols-3 gap-2">
                        {selectedEvent.best_odds.home && (
                          <button type="button" onClick={() => selectOutcome(`${selectedEvent.home_team} gana`, selectedEvent.best_odds.home!)}
                            className={`rounded-lg border px-2 py-2.5 text-xs transition-colors text-center ${form.selection === `${selectedEvent.home_team} gana` ? 'border-yellow-500/50 bg-yellow-500/10 text-yellow-300' : 'border-slate-600 hover:border-slate-500 text-slate-300'}`}>
                            <div className="font-semibold truncate">{selectedEvent.home_team.split(' ').pop()}</div>
                            <div className="text-yellow-400 font-black text-sm mt-1">{selectedEvent.best_odds.home.toFixed(2)}</div>
                          </button>
                        )}
                        {selectedEvent.best_odds.draw && (
                          <button type="button" onClick={() => selectOutcome('Empate', selectedEvent.best_odds.draw!)}
                            className={`rounded-lg border px-2 py-2.5 text-xs transition-colors text-center ${form.selection === 'Empate' ? 'border-yellow-500/50 bg-yellow-500/10 text-yellow-300' : 'border-slate-600 hover:border-slate-500 text-slate-300'}`}>
                            <div className="font-semibold">Empate</div>
                            <div className="text-yellow-400 font-black text-sm mt-1">{selectedEvent.best_odds.draw.toFixed(2)}</div>
                          </button>
                        )}
                        {selectedEvent.best_odds.away && (
                          <button type="button" onClick={() => selectOutcome(`${selectedEvent.away_team} gana`, selectedEvent.best_odds.away!)}
                            className={`rounded-lg border px-2 py-2.5 text-xs transition-colors text-center ${form.selection === `${selectedEvent.away_team} gana` ? 'border-yellow-500/50 bg-yellow-500/10 text-yellow-300' : 'border-slate-600 hover:border-slate-500 text-slate-300'}`}>
                            <div className="font-semibold truncate">{selectedEvent.away_team.split(' ').pop()}</div>
                            <div className="text-yellow-400 font-black text-sm mt-1">{selectedEvent.best_odds.away.toFixed(2)}</div>
                          </button>
                        )}
                      </div>
                      {form.selection && <p className="text-xs text-teal-400 mt-2">✓ {form.selection} @ {form.odds}</p>}
                    </div>
                  )}

                  {market === 'custom' && (
                    <div className="rounded-lg border border-slate-700 bg-slate-800 p-3 space-y-3">
                      <p className="text-xs text-slate-400">Escribe lo que apuestas — goleador, más/menos goles, ambos marcan, handicap...</p>
                      <div>
                        <label className="text-xs text-slate-500 mb-1 block">Tu apuesta *</label>
                        <input value={form.selection} onChange={e => setForm(f => ({ ...f, selection: e.target.value }))}
                          placeholder="Ej: Mbappé marca, Más de 2.5 goles, Ambos marcan..."
                          className="w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-white text-sm placeholder-slate-500 focus:border-yellow-500 focus:outline-none" required />
                      </div>
                      <div>
                        <label className="text-xs text-slate-500 mb-1 block">Cuota *</label>
                        <input type="number" step="0.01" min="1.01" value={form.odds} onChange={e => setForm(f => ({ ...f, odds: e.target.value }))}
                          placeholder="1.85"
                          className="w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-white text-sm placeholder-slate-500 focus:border-yellow-500 focus:outline-none" required />
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                ...
                <input value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))}
                  placeholder="Real Madrid vs Barça" required
              ...
              <div>
                ...
                <input type="datetime-local" value={form.match_date} onChange={e => setForm(f => ({ ...f, match_date: e.target.value }))}
                  className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-white text-sm focus:border-yellow-500 focus:outline-none" />
              </div>
              <div className="col-span-2">
                <label className="text-xs text-slate-400 mb-1 block">Tu apuesta * <span className="text-slate-600">(resultado, goleador, goles, lo que sea)</span></label>
                <input value={form.selection} onChange={e => setForm(f => ({ ...f, selection: e.target.value }))}
                  placeholder="Ej: Vinicius marca, Más de 2.5 goles, Bayern gana..." required
                  className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-white text-sm placeholder-slate-500 focus:border-yellow-500 focus:outline-none" />
              </div>
              <div>
          ...
          {/* ── APUESTA + NOTA (siempre al fondo) ── */}
          {(inputMode === 'manual' || (selectedEvent && form.selection)) && (
            <div className="space-y-3 pt-1 border-t border-slate-800">
              <div>
                <label className="text-xs text-slate-400 mb-1 block">¿Cuánto apuestas? (€ ficticio) *</label>
                <input type="number" min="1" max={Math.max(1, Math.floor(bankroll))} value={form.stake}
                  onChange={e => setForm(f => ({ ...f, stake: e.target.value }))} placeholder="50" required
                  className="w-full rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 text-white text-sm placeholder-slate-500 focus:border-yellow-500 focus:outline-none" />
              ...
              {form.odds && form.stake && !isNaN(+form.odds) && !isNaN(+form.stake) && (
                <div className="text-xs text-slate-400 bg-slate-800 rounded-lg px-3 py-2">
                  Si aciertas: <span className="text-green-400 font-semibold">+{fmt(+form.stake * (+form.odds - 1))}</span>
                  {' '}· Si fallas: <span className="text-red-400 font-semibold">-{fmt(+form.stake)}</span>
                </div>
              )}
            </div>
          )}
  ...
  return (
    <div className={`rounded-xl border p-4 ${
      ...
      <div className="flex items-center justify-between mt-3 pt-3 border-t border-slate-800/50">
        ...
        {isPending && onResolve && (
          <div className="flex gap-2">
            <button onClick={() => onResolve(pick.id, 'won')}
            ...
            <button onClick={() => onResolve(pick.id, 'lost')}
